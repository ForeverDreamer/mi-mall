<template>
	<view>
		...
		<view class="d-flex border-top border-bottom a-center" style="height: 100rpx;">
			<view class="d-flex flex-1 j-center a-center font-md"
			...
			<view class="d-flex flex-1 j-center a-center main-text-color"
			@tap="showRight = true">筛选</view>
		</view>
		<!-- 抽屉 -->
		<uni-drawer :visible="showRight" mode="right" @close="showRight = false">
			<card headTitle="服务" :headBorderBottom="false" :headTitleWeight="false">
				<!-- 单选按钮组 -->
				<sum-radio-group :label="label" :selected.sync="label.selected"></sum-radio-group>
			</card>
...
<script>
	...
	import sumRadioGroup from '@/components/common/sum-radio-group.vue'
